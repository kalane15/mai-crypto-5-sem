...
/**
 * MARS S-boxes for forward/backward mixing and key expansion.
 * - S0 and S1: 256-entry S-boxes (8-bit input to 32-bit output) for mixing phases
 * - S: 512-entry S-box (9-bit input to 32-bit output) for key expansion
 * The S-boxes are defined in the MARS specification Appendix A.
 * S is the concatenation of S0 and S1 (S0 = first 256 entries, S1 = next 256 entries).
 */
final class MarsSBox {

    // MARS S-box (512 entries total)
    // First 256 entries form S0, next 256 entries form S1
    ...
    /**
     * Gets S0 S-box value for an 8-bit index.
     * S0 is the first 256 entries of the S-box.
     * 
     * @param index 8-bit index (0-255)
     * @return 32-bit S-box value
     */
    static int getS0(int index) {
        if (index < 0 || index > 255) {
            throw new IllegalArgumentException("S0 index must be between 0 and 255");
        }
        return SBOX[index];
    }
    
    /**
     * Gets S1 S-box value for an 8-bit index.
     * S1 is entries 256-511 of the S-box.
     * 
     * @param index 8-bit index (0-255)
     * @return 32-bit S-box value
     */
    static int getS1(int index) {
        if (index < 0 || index > 255) {
            throw new IllegalArgumentException("S1 index must be between 0 and 255");
        }
        return SBOX[256 + index];
    }
    
    /**
     * Gets the S-box value for key expansion (9-bit index).
     * The S-box is 512 entries (concatenation of S0 and S1).
     * 
     * @param index 9-bit index (0-511)
     * @return 32-bit S-box value
     */
    static int getSBoxValue(int index) {
        if (index < 0 || index > 511) {
            throw new IllegalArgumentException("S-box index must be between 0 and 511");
        }
        return SBOX[index];
    }
}
